...
def _load(fname):
    spec = importlib.util.spec_from_file_location("k_" + Path(fname).stem, _DIR / fname)
...
_BENCH_SHAPES = [
    ("ID1_B128H8N8192D128_noncausal_fp16", 128, 8, 8, 8192, 128, False, torch.float16, 128, 128),
    ("ID2_B128H8N8192D64_noncausal_fp16", 128, 8, 8, 8192, 64, False, torch.float16, 128, 128),
    ("ID3_B128H8N1024D128_noncausal_fp16", 128, 8, 8, 1024, 128, False, torch.float16, 128, 128),
    ("ID4_B128H8N1024D64_noncausal_fp16", 128, 8, 8, 1024, 64, False, torch.float16, 128, 128),
    ("ID5_B128H8N8192D128_causal_fp16", 128, 8, 8, 8192, 128, True, torch.float16, 128, 128),
    ("ID6_B128H8N8192D64_causal_fp16", 128, 8, 8, 8192, 64, True, torch.float16, 128, 128),
    ("ID7_B128H8N1024D128_causal_fp16", 128, 8, 8, 1024, 128, True, torch.float16, 128, 128),
    ("ID8_B128H8N1024D64_causal_fp16", 128, 8, 8, 1024, 64, True, torch.float16, 128, 128),
]
...
def _manual_attention(q, k, v, sm_scale, causal):
    ...
    if causal:
        n_ctx = q.shape[-2]
        causal_mask = torch.ones((n_ctx, n_ctx), device=q.device, dtype=torch.bool).tril()
...
def _run_torch_ref(q, k, v, sm_scale, causal):
    ...
    try:
        if causal:
            ...
            n_ctx = q.shape[2]
            atten_mask = ~torch.ones(n_ctx, n_ctx, device=q.device, dtype=torch.bool).tril()
            out = torch_npu.npu_fusion_attention(
                q, k, v, head_num, "BNSD",
                padding_mask=None, atten_mask=atten_mask, scale=sm_scale,
                keep_prob=1.0, pre_tockens=65535, next_tockens=65535,
                sparse_mode=1,
            )
        else:
            out = torch_npu.npu_fusion_attention(
                q, k, v, head_num, "BNSD",
                padding_mask=None, atten_mask=None, scale=sm_scale,
                keep_prob=1.0, pre_tockens=65535, next_tockens=65535,
                sparse_mode=0,
            )
        return out[0] if isinstance(out, tuple) else out
    except Exception as e:
        print(f"INFO npu_fusion_attention_unavailable {type(e).__name__}; using manual reference")
...
def unit_test():
    print("=== Unit Test: flash_attention forward — every variant vs torch_ref ===")
    any_fail = False
    for label, *_ in _BENCH_SHAPES:
        ...
        for key, _name in _VARIANTS:
            ...
            if reason:
                print(
                    f"TEST {key} {label}: SKIP_UNSUPPORTED "
                    f"reason={reason} max_abs=inf mean_abs=inf"
                )
                continue
            try:
                with torch.no_grad():
                    out = _run_provider(key, q, k, v, sm_scale, causal, BLOCK_M, BLOCK_N)
                    _sync()
                max_abs = (ref.float() - out.float()).abs().max().item()
                mean_abs = (ref.float() - out.float()).abs().mean().item()
                ok = max_abs <= 2e-2 and mean_abs <= 2e-3
                status = "PASS" if ok else "FAIL"
                print(f"TEST {key} {label}: {status} max_abs={max_abs:.6e} mean_abs={mean_abs:.6e}")
                if not ok:
                    any_fail = True
            except Exception as e:
                msg = str(e).replace("\n", " ")
                print(f"TEST {key} {label}: ERROR {type(e).__name__} msg={msg} max_abs=inf mean_abs=inf")
                any_fail = True
    print("UNIT_TEST_FAILED" if any_fail else "UNIT_TEST PASS")
    return not any_fail


def _profiler_op_ms(fn, label, mode, op_type_filter=None,
                    wait=1, warmup=2, active=5):
    ...
    try:
        ...
        if op_avgs_us:
            ...
            print(f"INFO profiler_avg {mode} {label}: {avg_ms:.6f} ms from op_statistic dir={out_dir}")
            return avg_ms
        ...
        if durations:
            ...
            print(
                f"INFO profiler_median {mode} {label}: {median_ms:.6f} ms "
    ...
    try:
        with torch.no_grad():
            return _profiler_op_ms(
                lambda: _run_provider(mode, q, k, v, sm_scale, causal, BLOCK_M, BLOCK_N),
                label,
                mode,
...
def main():
    parser = argparse.ArgumentParser(description="Profile FlashAttention forward kernel")
