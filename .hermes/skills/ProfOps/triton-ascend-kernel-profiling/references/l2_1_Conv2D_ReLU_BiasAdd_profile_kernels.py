"""
profile_kernels.py — Conv2D → ReLU → BiasAdd  (l2_1_Conv2D_ReLU_BiasAdd)

Reference implementation of a profile_kernels.py for a post-conv activation
kernel on Ascend NPU. Demonstrates:
  - Loading baseline and optimized kernels via importlib (no copy-paste)
  - Three-way comparison: torch_ref (ACL) / baseline Triton / optimized Triton
  - Grid overflow guard for baseline (N*C*H > 65535 → chunk over N)
  - @triton.testing.perf_report for benchmark + figure
  - Shape coverage across both dispatch paths (persistent HW≤1024, loop HW>1024)
  - Unit test with fp16-appropriate tolerances

Kernel: relu(x) + bias  on [N, C, H, W] fp16  (post-conv2d activation)
Optimized kernel dispatch:
  HW ≤ 1024  → _relu_bias_persistent  (min(N*C, 32) grid-stride programs)
  HW >  1024  → _relu_bias_loop        ((N*C,) grid, BLOCK_HW=2048, stages=2)
...
# ── Shapes to benchmark ────────────────────────────────────────────────────────
# Covers both dispatch paths of the optimized kernel:
#   HW <= 1024  → persistent kernel (32-prog grid-stride, startup-amortised)
#   HW >  1024  → loop kernel       ((N*C,) grid, BLOCK=2048, num_stages=2)
_BENCH_SHAPES = [
...
# Prints table + saves relu_bias_add_perf.png automatically on .run().
# NOTE: styles must be (named_color, linestyle) — no hex codes, no marker suffixes.
# NOTE: do_bench returns seconds; perf_report ylabel says (ms) — no * 1e3 needed.
...
def main():
    parser = argparse.ArgumentParser(
        description="Profile Conv2D ReLU BiasAdd: torch_ref vs baseline vs optimized Triton")
