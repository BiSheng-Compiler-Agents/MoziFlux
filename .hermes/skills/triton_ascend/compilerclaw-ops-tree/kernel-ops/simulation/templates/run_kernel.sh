...
if [ ! -f "my_kernel.npubin" ]; then
    echo "[RUNNER] ERROR: npubin not found!"
    exit 1
fi
echo "[RUNNER] npubin size: $(stat -c%s my_kernel.npubin) bytes"
...
make -j$(nproc) 2>&1
cp test_kernel "$SCRIPT_DIR/"
cd "$SCRIPT_DIR"
echo "[RUNNER] Build OK, binary at: $SCRIPT_DIR/test_kernel"

# Step 4: Copy npubin to same dir as binary
cp my_kernel.npubin "$SCRIPT_DIR/my_kernel.npubin"
