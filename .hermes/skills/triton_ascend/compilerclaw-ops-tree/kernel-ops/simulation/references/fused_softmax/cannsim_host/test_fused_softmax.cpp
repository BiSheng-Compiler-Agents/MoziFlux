...
int main(int argc, char *argv[]) {
  ...
  CHECK_RT(rtMalloc(&outDev, dataSize, RT_MEMORY_HBM, 0),
           "rtMalloc out failed.");
  ...
  if (sum_ok)
    printf("[INFO] All rows sum to 1.0 (tol=1e-3)\n");

  if (correct)
    printf("[HOST] PASS — All %zu elements match reference (tol=1e-4)\n",
           numElem);
