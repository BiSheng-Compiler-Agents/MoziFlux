...
│   │   ├── templates/
│   │   │   ├── test_kernel_template.cpp   # Canonical sub-kernel host (K=1×BLOCK_K, _exit(0), [HOST] markers)
│   │   │   ├── compile_kernel_template.py # Canonical compile script
│   │   │   ├── cmake_template.txt         # Canonical CMakeLists.txt
│   │   │   └── run_kernel.sh              # Canonical build wrapper (sources CANN env)
│   │   └── references/
│   │       ├── vector_add/                # Simplest cannsim workflow example
│   │       ├── fused_softmax/             # Reduction + normalization example
│   │       ├── cannsim_subkernel_timeout.md # Timeout mitigation (K=1×BLOCK_K, smaller blocks)
│   │       ├── bf16-fp32-conversion.md    # bf16↔fp32 conversion helpers
│   │       ├── cannsim-cpp-host-dtype-matching.md # Kernel/host dtype matching
│   │       ├── cpp-host-build-pitfalls.md # GCC 14+ build issues
│   │       ├── rt-dev-binary-magic-elf.md # Binary magic constants
│   │       ├── cannsim-instr-bin-location.md # instr.bin location diagnosis
│   │       ├── subkernel-vs-multiblock-comparison.md # Sub-kernel vs multi-block
│   │       └── cannsim-compile-npubin-asm-dict.md # c.asm["npubin"] extraction
│   ├── profiling/SKILL.md                 # → profile_kernels.py, perf_report, NPU hardware benchmarking
│   │   └── templates/
│   │       └── profile_kernels.py         # Canonical profiling template (3-line single-baseline)
...
│   │   └── references/
│   │       ├── trace_comparison_methodology.md # Sub-kernel trace comparison
│   │       ├── two_phase_reduction_threshold.md # Two-phase reduction sweet spot (n_tiles > 64)
│   │       ├── grid-autotune-mismatch.md   # Grid/autotune BLOCK size mismatch
│   │       ├── pushq_vf_bottleneck_matmul.md # PUSHQ/VF bottleneck patterns
│   │       ├── row_scale_diagonal_matmul.md # Row-scale diagonal patterns
...
│       ├── triton-api-reference.md         # Complete Triton-Ascend API (779 lines): tl, al, bl, NPUOptions
...
│       ├── hardware-architecture.md        # AI Core architecture, memory hierarchy, platform diffs
...
| `triton-ascend-cannsim` | Sub-kernel timeout mitigation (K=1×BLOCK_K, smaller blocks) | `kernel-ops/simulation/SKILL.md` |
...
| `triton-operator-dev` | Full-process orchestration, 6-stage workflow | `triton-operator/orchestration/SKILL.md` |
...
| `triton-operator-performance-optim` | Two-path dispatch, diagonal scheduling, compounding effects | `kernel-ops/optimization/SKILL.md` |
...
| `triton-ascend-cannsim` | 6 | 1 |
| `triton-ascend-kernel-profiling` | 3 | 1 |
| `triton-operator-code-gen` | 3 | 1 |
