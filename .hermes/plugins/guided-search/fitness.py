...
def simulation_fitness(
    ...
    candidate_rate = _positive(candidate.get("useful_work"), "candidate useful_work") / _positive(
        candidate.get("wall_cycles"), "candidate wall_cycles"
    )
    baseline_rate = _positive(baseline.get("useful_work"), "baseline useful_work") / _positive(
...
def hardware_fitness(
    *, candidate_ms: Sequence[float], pytorch_acl_ms: Sequence[float]
) -> float:
    ...
    if not candidate_ms or len(candidate_ms) != len(pytorch_acl_ms):
        raise ValueError("candidate and PyTorch/ACL measurements must be non-empty and aligned")
    logs = []
