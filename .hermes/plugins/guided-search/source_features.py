...
_REDUCTION_CALLS = {
    "sum", "max", "min", "reduce", "associative_scan", "cumsum", "cumprod"
}
_NONLINEAR_CALLS = {
    "abs",
    "cos",
...
class SourceFeatures:
    ...
    def add(self, dimension: str, rule: str, node: ast.AST | None,
            detail: str) -> None:
...
def _import_aliases(tree: ast.AST) -> dict[str, str]:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            for item in node.names:
                aliases[item.asname
                        or item.name] = f"{node.module}.{item.name}"
    return aliases
...
def _is_cube_call(name: str) -> bool:
    ...
    leaf = _leaf(name)
    return (leaf in {
        "dot", "dot_scaled", "matmul", "mm", "bmm", "batch_matmul", "npu_bmmv2"
...
def _is_native_call(name: str) -> bool:
    lowered = name.lower()
    return (lowered.startswith("torch_npu.")
            or lowered.startswith("torch.nn.functional.") or "aclnn" in lowered
            or lowered in {"torch.bmm", "torch.matmul", "torch.mm"})


def _native_module_attributes(tree: ast.AST,
...
def _function_index(
        tree: ast.AST) -> dict[str, ast.FunctionDef | ast.AsyncFunctionDef]:
...
def _reachable_functions(
    ...
    if not roots:
        roots = {
            ...
            if any("triton.jit" in _call_name(decorator.func if isinstance(
...
def _nodes_for(functions: Mapping[str, ast.AST],
               reachable: Iterable[str]) -> list[ast.AST]:
    nodes: list[ast.AST] = []
...
def _expanded_call_leaves(node: ast.AST,
                          functions: Mapping[str, ast.AST]) -> set[str]:
    leaves: set[str] = set()
    pending = [node]
...
def _assigned_names(
        node: ast.Assign | ast.AnnAssign | ast.AugAssign) -> set[str]:
...
def _numerical_accumulator(
        nodes: Iterable[ast.AST]) -> tuple[bool, ast.AST | None]:
    nodes = list(nodes)
    for node in nodes:
    ...
    for loop in loops:
        ...
        for node in ast.walk(loop):
            ...
            meaningful_name = any(
                name.lower() in _ACCUMULATOR_NAMES or name.lower().startswith((
                    "acc", "sum", "running_")) for name in self_carried)
            numerical_value = any(
...
def _kernel_launch(node: ast.Call, functions: Mapping[str,
                                                      ast.AST]) -> str | None:
...
def _branch_launches(statements: Iterable[ast.stmt],
                     functions: Mapping[str, ast.AST]) -> set[str]:
...
def _shape_or_regime_test(test: ast.AST) -> bool:
    ...
    has_regime = any(
        (isinstance(node, ast.Attribute)
         and node.attr in {"shape", "dtype", "ndim", "numel"}) or (
             isinstance(node, ast.Name) and node.id.lower() in {
                 "b",
                 "c",
                 "d",
                 "h",
...
def extract_source_features(
    ...
    *,
    ...
    nodes = (_nodes_for(functions, reachable) if functions else list(
        ast.walk(tree)))
    features = SourceFeatures()

    aliases = _import_aliases(tree)
    call_records = [(_resolve_alias(_call_name(node.func), aliases), node)
    ...
    cube_records = [(name, node) for name, node in call_records
                    if _is_cube_call(name)]
    features.cube_ops = (len(cube_records) + len(native_cube_records) +
                         len(matmul_nodes))
    if cube_records or native_cube_records or matmul_nodes:
        node = (cube_records[0][1]
                if cube_records else native_cube_records[0][1]
                if native_cube_records else matmul_nodes[0])
        features.add("engine", "cube_matmul", node,
                     "reachable dot/BMM/matmul is assigned to Cube")

    vector_records = [(name, node) for name, node in call_records
    ...
    reachable_kernel_names.update(name for name in reachable if any(
        "triton.jit" in _call_name(
    ...
    for name in sorted(reachable_kernel_names):
        ...
        leaves = {
            _leaf(_call_name(node.func))
            for node in function_nodes if isinstance(node, ast.Call)
        }
        families = {
            family
            for family, present in {
                "cube":
                any(
                    _is_cube_call(_call_name(node.func))
                    for node in function_nodes if isinstance(node, ast.Call)),
                "reduction":
                bool(leaves & _REDUCTION_CALLS),
                "nonlinear":
                bool(leaves & _NONLINEAR_CALLS),
                "atomic":
                any(leaf.startswith("atomic_") for leaf in leaves),
            }.items() if present
        }
        if len(families) >= 2:
            features.fused_kernel = True
            features.add(
                "algorithm", "fused_kernel", functions[name],
    ...
    features.scan = bool(call_leaves
                         & {"associative_scan", "cumsum", "cumprod"})
    online_node = next(
        (node for node in loop_nodes if {"max", "exp", "sum"}.issubset(
            _expanded_call_leaves(node, functions))), None)
    ...
    if features.multi_stage:
        features.add(
            "algorithm", "multi_stage_decomposition", None,
    ...
    features.compiler_managed = bool(enabled_options
                                     & compiler_managed_options)
    ...
    if contiguous_calls:
        features.add(
            "memory", "contiguous_aligned_access", contiguous_calls[0],
            "reachable code asserts or materializes contiguous/aligned access")

    tiling_calls = [
        node for name, node in call_records if _leaf(name) in
        {"make_block_ptr", "advance", "alloc", "compile_hint"}
    ]
    swizzle_names = {
        node.id
        for node in nodes if isinstance(node, ast.Name) and node.id.upper() in
        {"GROUP_M", "GROUP_N", "NUM_PID_M", "NUM_PID_N"}
    }
    features.explicit_tiling = bool(
        tiling_calls or swizzle_names
        or enabled_options & (memory_tiling_options | memory_reuse_options))
    if features.explicit_tiling:
        features.add(
            "memory", "explicit_tile_or_reuse",
            tiling_calls[0] if tiling_calls else None,
    ...
    if features.buffered_pipeline:
        features.add(
            "memory", "buffered_hierarchy",
            buffering_calls[0] if buffering_calls else None,
    ...
    for name, node in call_records:
        ...
        if leaf == "scope":
            ...
            for keyword in node.keywords:
                if keyword.arg == "core_mode" and isinstance(
        ...
        if leaf in {"alloc", "sub_vec_id"} and name.startswith(("bl.", "al.")):
            manual_nodes.append(node)
        if leaf in {
                "compile_hint", "multibuffer", "cast", "extract_slice",
                "insert_slice"
        }:
    ...
    if features.explicit_cv_handoff:
        features.add(
            "engine", "explicit_cv_handoff",
            sync_nodes[0] if sync_nodes else None,
            "reachable Cube/Vector scopes or synchronization form a handoff")
    features.overlapped_cv_pipeline = bool(
        features.explicit_cv_handoff and
        (features.buffered_pipeline or enabled_options & coordination_options))
    if features.overlapped_cv_pipeline:
        features.add(
            "engine", "overlapped_cv_pipeline", None,
    ...
    if features.tuned_direct:
        features.add(
            "dispatch", "tuned_or_swizzled",
            autotune_nodes[0] if autotune_nodes else None,
    ...
    persistent_node = next(
        ...
        None)
    persistent_name = next(
        (functions[name]
         for name in reachable if "persistent" in name.lower()), None)
    features.persistent = bool(persistent_node or persistent_name
                               or enabled_options & dispatch_options)
    if features.persistent:
        features.add("dispatch", "persistent_or_grid_stride", persistent_node
                     or persistent_name,
    ...
    for name in sorted(reachable):
        ...
        for node, fallback in candidate_ifs:
            ...
            if body and other and body != other and _shape_or_regime_test(
                    node.test):
    ...
    if features.multi_path:
        features.add(
            "dispatch", "shape_regime_multi_path", multi_path_node,
    ...
    if environment.get("TRITON_ALL_BLOCKS_PARALLEL"):
        ...
        features.add(
            "dispatch", "all_blocks_parallel", None,
